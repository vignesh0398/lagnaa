...
export function Sidebar() {
  ...
  return (
    <aside className="fixed left-0 top-0 z-40 flex h-screen w-64 flex-col border-r border-white/5 bg-surface-900/80 backdrop-blur-2xl">
      <NavLink
        to="/home"
      ...
      <nav className="flex-1 space-y-1 overflow-y-auto px-3 py-4">
        ...
        {[...channelGroups, marketingGroup].map((group) => {
        ...
        <button
          ...
          className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition-[background-color,color] duration-100 ease-out ${
            settingsOpen || settingsItems.some((i) => pathname === i.to || pathname.startsWith(`${i.to}/`))
              ? 'bg-white/[0.07] text-white'
              : 'text-slate-300 hover:bg-white/5 hover:text-white'
          }`}
        >
